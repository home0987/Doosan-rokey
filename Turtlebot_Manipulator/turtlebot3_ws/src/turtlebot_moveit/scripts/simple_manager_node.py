...
class IntegratedProcess(Node):
        ...
        if not self.armrun:  # 로봇 암이 동작 중이 아니면
            ...
            try:
                ...
                if self.yolo_x > 0 and self.yolo_y > 0:   # right low
                    self.yolo_y = self.yolo_x + right_low_y_offset       # hight  += 아래   -= 위
                    self.yolo_x = self.yolo_y + right_low_x_offset       # width  += 오른쪽 -= 왼쪽
                    print(f'right low  :  yolo_robot_x [{self.yolo_x}] yolo_robot_y[{self.yolo_y}]')
                # x : 0.03522528820800782] y : 0.0352 2528820800782]
                elif self.yolo_x > 0 and self.yolo_y < 0:  # right high
                    self.yolo_y = self.yolo_x + right_high_y_offset       # hight  += 아래   -= 위
                    self.yolo_x = self.yolo_y + right_high_x_offset       # width  += 오른쪽 -= 왼쪽
                    #print("right high")
                    print(f'right high  :  yolo_robot_x [{self.yolo_x}] yolo_robot_y[{self.yolo_y}]')
                elif self.yolo_x < 0 and self.yolo_y > 0:  # left low
                    self.yolo_y = self.yolo_x + left_low_y_offset       # hight  += 아래   -= 위
                    self.yolo_x = self.yolo_y + left_low_x_offset       # width  += 오른쪽 -= 왼쪽
                    #print("left low")
                    print(f'left low  :  yolo_robot_x [{self.yolo_x}] yolo_robot_y[{self.yolo_y}]')
                elif self.yolo_x < 0 and self.yolo_y < 0:  # left high
                    self.yolo_y = self.yolo_x + left_high_y_offset       # hight  += 아래   -= 위
                    self.yolo_x = self.yolo_y + left_high_x_offset       # width  += 오른쪽 -= 왼쪽
                    #print("left high")
    ...
    def yolo_arm_controll(self):
        ...
        if self.yolofind:
            ...
            pose_array = self.append_pose_init(0.14 - self.yolo_x + 0.055 + 0.01, 0.0 - self.yolo_y * 1.2, 0.122354 )
